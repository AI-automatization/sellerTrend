...
## SECURITY AUDIT (2026-05-08) — GitHub Issues #1-15

> Manba: Tezcode Audit System tomonidan yuborilgan GitHub issues
> Barcha tasklarni bajarishdan oldin `git pull origin main` qiling

---

### T-526 | P0 | BACKEND | Fix hardcoded credentials in SeedService | 30min

**Sana:** 2026-05-10
**Manba:** kod-audit
**Topilgan joyda:** `apps/api/src/common/seed.service.ts:44,57`
**Mas'ul:** Sardor
**GitHub:** AI-automatization/sellerTrend#1

**Tahlil:**
SeedService da admin va demo accountlar uchun parollar hardcoded. Repo ga kirish huquqi bo'lgan har kim super admin parolini biladi. Git history da ham saqlanib qolgan.

**Muammo:** Production tizimga to'liq kirish imkoniyati — CRITICAL.

**Yechim:**
`SEED_ADMIN_PASSWORD` va `SEED_DEMO_PASSWORD` env variablelardan o'qish. `.env.example` da placeholder allaqachon bor — faqat kod ishlatmayapti. Tuzatgandan keyin production da parolni force-rotate qilish.

**Fayllar:**
```
apps/api/src/common/seed.service.ts   → env dan o'qish
```

---

### T-527 | P0 | BACKEND | Disable Swagger in production | 15min

**Sana:** 2026-05-10
**Manba:** kod-audit
**Topilgan joyda:** `apps/api/src/main.ts:85-94`
**Mas'ul:** Sardor
**GitHub:** AI-automatization/sellerTrend#2

**Tahlil:**
`/api/docs` va `/api-json` production da hech qanday autentifikatsiyasiz ochiq. Hamma endpoint, schema va auth mexanizmlari ko'rinib turadi.

**Muammo:** Hujumchi uchun to'liq API xaritasi — boshqa zaifliklarni topishni osonlashtiradi.

**Yechim:**
Swagger setup ni `NODE_ENV !== 'production'` sharti ostiga olish.

**Fayllar:**
```
apps/api/src/main.ts   → NODE_ENV tekshiruvi qo'shish
```

---

### T-528 | P0 | FRONTEND | Fix XSS in ChatMessage dangerouslySetInnerHTML | 1h

**Sana:** 2026-05-10
**Manba:** kod-audit
**Topilgan joyda:** `apps/web/src/components/chat/ChatMessage.tsx:30-38,62`
**Mas'ul:** Sardor
**GitHub:** AI-automatization/sellerTrend#3

**Tahlil:**
`renderMarkdown` funksiyasi faqat regex replacement qiladi, sanitize qilmaydi. AI javobi yoki manipulyatsiya qilingan WebSocket payload orqali `<img onerror=alert(...)>` kabi kod browserda ishlaydi.

**Muammo:** Session hijacking, account takeover, dashboard dan ma'lumot o'g'irlash.

**Yechim:**
`dompurify` kutubxonasini qo'shib `DOMPurify.sanitize()` ishlatish, yoki `react-markdown` ga o'tish (tavsiya etiladi).

**Fayllar:**
```
apps/web/src/components/chat/ChatMessage.tsx   → sanitize qo'shish
```

---


### T-531 | P1 | BACKEND | Add helmet middleware for security headers | 15min

**Sana:** 2026-05-10
**Manba:** kod-audit
**Topilgan joyda:** `apps/api/src/main.ts:1-12`
**Mas'ul:** Sardor
**GitHub:** AI-automatization/sellerTrend#6

**Tahlil:**
`helmet` middleware yo'q — `X-Frame-Options`, `X-Content-Type-Options`, `CSP` va boshqa security headerlar qaytarilmayapti. Clickjacking va MIME sniffing xurujlariga ochiq.

**Yechim:**
`app.use(helmet())` qo'shish. `helmet` allaqachon NestJS bilan keladi.

**Fayllar:**
```
apps/api/src/main.ts   → helmet() qo'shish
```

---

### T-532 | P1 | BACKEND | Remove tmp_fix_photos.mjs with hardcoded DB string | 15min

**Sana:** 2026-05-10
**Manba:** kod-audit
**Topilgan joyda:** `tmp_fix_photos.mjs` (repo root)
**Mas'ul:** Sardor
**GitHub:** AI-automatization/sellerTrend#7

**Tahlil:**
Repo root da vaqtinchalik script qoldirilgan, ichida hardcoded production DB connection string bor. Repo ga kirish huquqi bo'lgan har kim DB ga to'g'ridan ulanishi mumkin.

**Yechim:**
Faylni o'chirish. Git history dan ham tozalash kerak (`git filter-branch` yoki BFG). DB parolini rotate qilish.

**Fayllar:**
```
tmp_fix_photos.mjs   → o'chirish
```
...
### T-534 | P2 | BACKEND | Replace $queryRawUnsafe in auth.service.ts | 30min

**Sana:** 2026-05-10
**Manba:** kod-audit
**Topilgan joyda:** `apps/api/src/auth/auth.service.ts:~195-198`
**Mas'ul:** Sardor
**GitHub:** AI-automatization/sellerTrend#9

**Tahlil:**
`$queryRawUnsafe` SQL injection xavfini tug'diradi. Prisma ning `$queryRaw` tagged template yoki typed query ishlatilishi kerak.

**Yechim:**
`$queryRaw\`...\`` tagged template literal ga o'tish.

**Fayllar:**
```
apps/api/src/auth/auth.service.ts   → $queryRawUnsafe → $queryRaw
```

---


### T-536 | P2 | DEVOPS | Fix pnpm audit failures silently ignored in CI | 15min

**Sana:** 2026-05-10
**Manba:** kod-audit
**Topilgan joyda:** `.github/workflows/ci.yml`
**Mas'ul:** Sardor
**GitHub:** AI-automatization/sellerTrend#11

**Tahlil:**
CI da `pnpm audit` xatolari `|| true` bilan ignore qilinmoqda. Zaif dependency lar production ga o'tib ketishi mumkin.

**Yechim:**
`|| true` ni olib tashlash. HIGH/CRITICAL vulnerability larda CI fail bo'lishi kerak.

**Fayllar:**
```
.github/workflows/ci.yml   → audit failure ni bloklovchi qilish
```

---

### T-537 | P2 | BACKEND | Fix CORS — remove chrome-extension wildcard | 15min

**Sana:** 2026-05-10
**Manba:** kod-audit
**Topilgan joyda:** `apps/api/src/main.ts:72-77`
**Mas'ul:** Sardor
**GitHub:** AI-automatization/sellerTrend#12

**Tahlil:**
CORS config da `chrome-extension://` wildcard origin ruxsat berilgan. Har qanday Chrome extension API ga so'rov yuborishi mumkin.

**Yechim:**
Faqat o'ziga tegishli extension ID ni whitelist qilish.

**Fayllar:**
```
apps/api/src/main.ts   → CORS origin aniqlashtirish
```

---

### T-538 | P2 | BACKEND | Remove password reset token from logs | 15min

**Sana:** 2026-05-10
**Manba:** kod-audit
**Topilgan joyda:** `apps/api/src/auth/auth.service.ts`
**Mas'ul:** Sardor
**GitHub:** AI-automatization/sellerTrend#13

**Tahlil:**
Password reset token plaintext holatda log ga yozilmoqda. Log fayllariga kirish huquqi bo'lgan har kim tokenni ishlatib account ni egallashi mumkin.

**Yechim:**
Log dan token ni olib tashlash yoki `token.substring(0,8)+'...'` ko'rinishida qisqartirish.

**Fayllar:**
```
apps/api/src/auth/auth.service.ts   → token logging olib tashlash
```

---

### T-539 | P2 | BACKEND | Replace $executeRawUnsafe in SeedService | 15min

**Sana:** 2026-05-10
**Manba:** kod-audit
**Topilgan joyda:** `apps/api/src/common/seed.service.ts`
**Mas'ul:** Sardor
**GitHub:** AI-automatization/sellerTrend#14

**Tahlil:**
pgvector extension yaratish uchun `$executeRawUnsafe` ishlatilmoqda. `$executeRaw` tagged template bilan almashtirilishi kerak.

**Fayllar:**
```
apps/api/src/common/seed.service.ts   → $executeRawUnsafe → $executeRaw
```
...
**Sana:** 2026-05-10
**Manba:** kod-audit
**Topilgan joyda:** `.env.example`
**Mas'ul:** Sardor
**GitHub:** AI-automatization/sellerTrend#15

**Tahlil:**
`.env.example` da `JWT_SECRET=your-secret-key` kabi oson taxmin qilinadigan qiymatlar bor. Yangi developer bu qiymatni o'zgartirmasdan ishlatishi mumkin.

**Yechim:**
`.env.example` da `JWT_SECRET=` (bo'sh) yoki generatsiya qilish instruksiyasi qo'yish. Localhost CORS ni production config dan ajratish.

**Fayllar:**
```
.env.example   → placeholder larni aniqlashtirish
```

---

*Tasks.md | VENTRA Analytics Platform | 2026-05-10*
