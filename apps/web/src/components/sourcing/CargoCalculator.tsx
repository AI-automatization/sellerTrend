...
export function CargoCalculator({
  ...
  // Rates dastlab null bo'ladi — kelganda prefilldan qayta hisoblash
  useEffect(() => {
    if (rates && prefillItemCostUzs && !ratesAppliedRef.current) {
      ratesAppliedRef.current = true;
      setForm((f) => ({ ...f, item_cost_usd: (prefillItemCostUzs / rates.USD).toFixed(2) }));
    }
  }, [rates]);
