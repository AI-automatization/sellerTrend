import { useState, useEffect } from 'react';
import { useI18n } from '../i18n/I18nContext';
import { referralApi } from '../api/client';
import { logError, toastError } from '../utils/handleError';

interface ReferralStats {
  my_code: string | null;
  total_referred: number;
  active: number;
  earned_days: number;
}

export function ReferralPage() {
  const [stats, setStats] = useState<ReferralStats | null>(null);
  const [loading, setLoading] = useState(true);
  const [generating, setGenerating] = useState(false);
  const [copied, setCopied] = useState(false);
  const { t } = useI18n();

  useEffect(() => {
    referralApi.getStats()
      .then((r) => setStats(r.data))
      .catch(logError)
      .finally(() => setLoading(false));
  }, []);

  async function handleGenerate() {
    setGenerating(true);
    try {
      const res = await referralApi.generateCode();
      setStats((s) => s ? { ...s, my_code: res.data.code } : s);
    } catch (e) { toastError(e); }
    finally { setGenerating(false); }
  }

  function copyLink() {
    if (!stats?.my_code) return;
    const link = `${window.location.origin}/register?ref=${stats.my_code}`;
    navigator.clipboard.writeText(link);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  }

  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <span className="loading loading-dots loading-lg text-primary" />
      </div>
    );
  }

  return (
    <div className="w-full space-y-4 lg:space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl lg:text-3xl font-bold flex items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 lg:w-7 lg:h-7 text-secondary">
            <path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" />
          </svg>
          {t('referral.title')}
        </h1>
        <p className="text-base-content/50 text-sm mt-1">
          {t('referral.subtitle')}
        </p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="stat bg-base-200/60 border border-base-300/50 rounded-2xl">
          <div className="stat-title text-xs">{t('referral.stat.invited')}</div>
          <div className="stat-value text-xl">{stats?.total_referred ?? 0}</div>
          <div className="stat-desc">{t('referral.stat.total')}</div>
        </div>
        <div className="stat bg-base-200/60 border border-base-300/50 rounded-2xl">
          <div className="stat-title text-xs">{t('referral.stat.active')}</div>
          <div className="stat-value text-xl text-success">{stats?.active ?? 0}</div>
          <div className="stat-desc">{t('referral.stat.registered')}</div>
        </div>
        <div className="stat bg-base-200/60 border border-base-300/50 rounded-2xl">
          <div className="stat-title text-xs">{t('referral.stat.earnedDays')}</div>
          <div className="stat-value text-xl text-primary">{stats?.earned_days ?? 0}</div>
          <div className="stat-desc">{t('referral.stat.freeDays')}</div>
        </div>
        <div className="stat bg-base-200/60 border border-base-300/50 rounded-2xl">
          <div className="stat-title text-xs">{t('referral.stat.perReferral')}</div>
          <div className="stat-value text-xl">7</div>
          <div className="stat-desc">{t('referral.stat.freeDaysUnit')}</div>
        </div>
      </div>

      {/* Code card */}
      <div className="rounded-2xl bg-base-200/60 border border-base-300/50">
        <div className="card-body">
          <h2 className="card-title text-base">{t('referral.codeCard.title')}</h2>

          {stats?.my_code ? (
            <div className="space-y-4">
              <div className="bg-base-300 rounded-xl p-4 flex items-center justify-between">
                <div>
                  <p className="text-xs text-base-content/40">{t('referral.codeCard.label')}</p>
                  <p className="text-2xl font-mono font-bold tracking-widest">{stats.my_code}</p>
                </div>
                <button onClick={copyLink} className="btn btn-primary btn-sm">
                  {copied ? t('referral.codeCard.copied') : t('referral.codeCard.copyLink')}
                </button>
              </div>

              <div className="bg-base-300 rounded-xl p-4">
                <p className="text-xs text-base-content/40 mb-1">{t('referral.codeCard.linkLabel')}</p>
                <p className="text-sm font-mono break-all">
                  {window.location.origin}/register?ref={stats.my_code}
                </p>
              </div>

              <p className="text-xs text-base-content/40">
                {t('referral.linkDescription')}
              </p>
            </div>
          ) : (
            <div className="flex flex-col items-center py-6 gap-4">
              <p className="text-base-content/50 text-sm text-center">
                {t('referral.noCodeYet')}
              </p>
              <button onClick={handleGenerate} disabled={generating} className="btn btn-primary">
                {generating && <span className="loading loading-spinner loading-sm" />}
                {generating ? t('referral.creatingBtn') : t('referral.createBtn')}
              </button>
            </div>
          )}
        </div>
      </div>

      {/* How it works */}
      <div className="rounded-2xl bg-base-200/60 border border-base-300/50">
        <div className="card-body">
          <h2 className="card-title text-base">{t('referral.howTitle')}</h2>
          <ul className="steps steps-vertical text-sm">
            <li className="step step-primary">{t('referral.step1')}</li>
            <li className="step step-primary">{t('referral.step2')}</li>
            <li className="step step-primary">{t('referral.step3')}</li>
            <li className="step step-primary">{t('referral.step4')}</li>
          </ul>
        </div>
      </div>
    </div>
  );
}
